...
We design `MyApp` with this consideration. With `MyApp` you will have a straightforward development experience like building a simple monolith. But at the end, you know that `MyApp` is always ready for microservices.
...
Now let's dive into the architecture; so that you can get a better picture of why `myApp` is probably a good solution to your use case.
...
Now you want to run `MyApp` as microservices.
