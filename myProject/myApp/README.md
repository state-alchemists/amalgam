...
`MyApp` is created by using [Zaruba](https://github.com/state-alchemists/zaruba).

To generate `MyApp`, you can use the following command:
...
Zaruba also allows you to add custom resources (like modules or API endpoints) into `MyApp`. For this to work, you need to follow a few conventions:
...
Once running, you can access `MyApp` by pointing your browser to [http://localhost:3000](http://localhost:3000).
...
To run the `myApp` test, you can invoke the following command:
