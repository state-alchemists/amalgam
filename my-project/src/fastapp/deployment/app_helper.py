...
image = os.getenv('IMAGE', 'fastapp:latest')
app_monolith_replica = int(os.getenv('REPLICA', '1'))
app_monolith_labels = {'app': 'fastapp'}
app_gateway_replica = int(os.getenv('REPLICA_GATEWAY'))
app_gateway_labels = {'app': 'fastapp-gateway'}
...
def create_app_microservices_deployments() -> List[k8s.apps.v1.Deployment]:
    ...
    for module in MODULES:
        ...
        app_service_labels = {
            'app': f'fastapp-{kebab_module_name}-service'
        }
        deployments.append(_create_app_deployment(
            resource_name=f'fastapp-{kebab_module_name}-service',
