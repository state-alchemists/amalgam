...
image = os.getenv("IMAGE", "myapp:latest")
app_monolith_replica = int(os.getenv("REPLICA", "1"))
app_monolith_labels = {"app": "myapp"}
app_gateway_replica = int(os.getenv("REPLICA_GATEWAY"))
app_gateway_labels = {"app": "myapp-gateway"}
...
def create_app_microservices_deployments() -> list[k8s.apps.v1.Deployment]:
    ...
    for module in MODULES:
        ...
        app_service_labels = {"app": f"myapp-{kebab_module_name}-service"}
        deployments.append(
            _create_app_deployment(
                resource_name=f"myapp-{kebab_module_name}-service",
