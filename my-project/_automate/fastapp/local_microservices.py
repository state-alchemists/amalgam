...
def get_start_microservices(upstreams: List[Task]) -> List[Task]:
    ...
    for index, module in enumerate(MODULES):
        ...
        service_template_env_file = EnvFile(
            ...
            prefix=f'FASTAPP_{upper_snake_module_name}'
        )
        service_env_files = [service_template_env_file]
        # Create service_envs
        service_port_str = str(3000 + index + 1)
        service_app_port_env = Env(
            name='APP_PORT',
            os_name=f'FASTAPP_{upper_snake_module_name}_APP_PORT',
        ...
        service_checker = HTTPChecker(
            name=f'check-fastapp-{kebab_module_name}-service',
        ...
        start_service = CmdTask(
            name=f'start-fastapp-{kebab_module_name}-service',
            inputs=[
                local_input,
                mode_input,
                host_input,
