...
def get_start_microservices(upstreams: List[Task]) -> List[Task]:
    ...
    for module_index, module_name in enumerate(MODULES):
        ...
        start_service = CmdTask(
            name=f"start-myapp-{kebab_module_name}-service",
            inputs=[
                local_input,
                run_mode_input,
                host_input,
            ...
            + _get_service_envs(3000, module_index, module_name),
            cmd_path=[
            ...
            checkers=[
                HTTPChecker(
                    name=f"check-myapp-{kebab_module_name}-service",
...
def _get_service_env_file(module_name: str) -> EnvFile:
    ...
    return EnvFile(
        ...
        prefix=f"MYAPP_{upper_snake_module_name}",
    )
